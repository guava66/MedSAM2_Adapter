...
def get_random_points_as_4tuple(mask, num_points=2):
    # 获取 mask 中所有非零点的坐标
    y_indices, x_indices = np.where(mask > 0)
    
    # 如果 mask 中没有非零点，返回空数组
    if len(x_indices) == 0:
        return np.array([])
    
    # 随机选取 num_points 个点的索引
    random_indices = np.random.choice(len(x_indices), size=num_points, replace=False)
...
def get_random_points_as_6tuple(mask, num_points=3):
    # 获取 mask 中所有非零点的坐标
    y_indices, x_indices = np.where(mask > 0)
    ...
    # 将三个点转换为 6 元组 [x1, y1, x2, y2, x3, y3]
    x1, y1 = random_points[0]
    x2, y2 = random_points[1]
    x3, y3 = random_points[2]
...
def medsam_inference(
    ...
    if len(box_torch.shape) == 2:
        box_coords = box_torch.reshape(-1, 2,2) # (B, 4) to (B, 2, 2)
        box_labels = torch.tensor([[1,1]], dtype=torch.int, device=img_embed.device)
        box_labels = box_labels.repeat(box_torch.size(0), 1)
    concat_points = (box_coords, box_labels)
...
parser.add_argument(
    "-data_root",
    type=str,
    default="data/npz_train/epilepsy",
    help="Path to the data folder",
)
parser.add_argument(
    "-pred_save_dir",
    type=str,
    default="/data/pyhData/MedSAM-MedSAM2/work_dir/MedSAM2-after_norm_residual-adapter-2-20250407-2121",
...
parser.add_argument(
    "-medsam2_checkpoint",
    type=str,
    default="/data/pyhData/MedSAM-MedSAM2/work_dir/MedSAM2-after_norm_residual-adapter-2-20250407-2121/medsam_model_best.pth",
...
parser.add_argument("-data_name",type=str,default="epilepsy")
args = parser.parse_args()

visualize = args.visualize
data_root = args.data_root
pred_save_dir = join(args.pred_save_dir,"result",args.data_name)
...
sam2_model = build_sam2(model_cfg, sam2_checkpoint, device=device, mode="eval", apply_postprocessing=True,adapter=True)
...
def main(file_names):
    ...
    for name in file_names:
        ...
        for label_id in label_ids:
            ...
            marker_zids = np.sort(np.unique(marker_zids))#从小到大（0，1，3，4，6，7，8，。。。）
            bbox_dict = {} # key: z_index, value: bbox
            for z in marker_zids:
                z_box = get_bbox(marker_data_id[z, :, :])#只是标签值
                bbox_dict[z] = z_box
            # find largest bbox in bbox_dict
            bbox_areas = [np.prod(bbox_dict[z][2:] - bbox_dict[z][:2]) for z in bbox_dict.keys()]
            z_max_area = list(bbox_dict.keys())[np.argmax(bbox_areas)]#为最大面积对应的z
            z_min = min(bbox_dict.keys())
            z_max = max(bbox_dict.keys())
            z_max_area_bbox = mid_slice_bbox_2d = bbox_dict[z_max_area]

            z_middle = int((z_max - z_min)/2 + z_min)
            # 提取 z_middle 切片的图像和边界框
            img_2d_middle = img_3D[z_middle, :, :]
            mid_slice_bbox_2d = bbox_dict[z_middle]  # 获取 z_middle 切片的边界框
            ...
            for z in range(z_middle, z_max):
                ...
                if len(img_2d.shape) == 2:
                    img_3c = np.repeat(img_2d[:, :, None], 3, axis=-1)# 3, axis=-1)：沿着最后一个维度（颜色通道）复制三次
                else:
                    img_3c = img_2d
                H, W, _ = img_3c.shape
                ...
                if z == z_middle:
                    ...
                    if np.max(pre_seg1024) > 0:#检查上一层分割图像中是否有有效的分割结果（即是否有像素值大于 0）。如果上一层有有效分割，那么我们使用这个分割结果的边界框作为当前切片的预测区域。
                        ...
                        box_1024_1=get_random_points_as_4tuple(pre_seg1024)
                    else:
                        pre_seg = segs_3d_temp[z-2, :, :]
                        pre_seg1024 = cv2.resize(pre_seg, (1024, 1024), interpolation=cv2.INTER_NEAREST)
                        if np.max(pre_seg1024) > 0:#检查上一层分割图像中是否有有效的分割结果（即是否有像素值大于 0）。如果上一层有有效分割，那么我们使用这个分割结果的边界框作为当前切片的预测区域。
                            box_1024 = get_bbox(pre_seg1024)
                            box_1024_1=get_random_points_as_4tuple(pre_seg1024)
                ...
                # # 显示边界框
                # show_box(box_1024, plt.gca())  # 在当前图像上绘制边界框
                # plt.axis("off")
                # plt.title(f"z_middle Slice (z={z_middle}) with Bounding Box")
                # plt.show()
                # #plt.savefig(f"./work_dir/MedSAM2-Tiny-Flare22-20250117-2318/z={z}data_sanitycheck.png", bbox_inches="tight", dpi=300)
    
                img_2d_seg = medsam_inference(medsam_model, _features, box_1024[None,:], H, W)
                # if z==33 or z==34:
                #     # 可视化当前切片的图像、边界框和 mask
                #     plt.figure(figsize=(10, 10))
                #     plt.imshow(img_3c)

                #     # 显示模型输出的 mask
                #     show_mask(img_2d_seg, plt.gca())  # 在当前图像上绘制 mask

                #     # 显示边界框
                #     show_box(mid_slice_bbox_2d, plt.gca())  # 在当前图像上绘制边界框

                #     plt.axis("off")
                #     plt.title(f"Slice (z={z}) with Mask and Bounding Box")
                #     plt.show()
                #     plt.savefig(f"./work_dir/33-34/z={z}mask.png", bbox_inches="tight", dpi=300)
                ...
                # print(f"Dice:{1-seg_loss(pred_2d, gt_2d)}")
                this_iou += iou
                this_dice+=dice
                num_slices += 1
            ...
            for z in range(z_middle-1, z_min, -1):
                ...
                # plt.title(f"Slice (z={z}) with Mask and Bounding Box")
                # plt.show()
                # plt.savefig(f"./work_dir/MedSAM2-Tiny-Flare22-20250117-2318/z={z}mask.png", bbox_inches="tight", dpi=300)
                segs_3d_temp[z, img_2d_seg>0] = 1
                # 计算当前切片的损失
                gt_2d = gt_3D[z, :, :] == label_id
                gt_2d = torch.from_numpy(gt_2d).float().to(device)
        ...
        # print(f"File: {name}, Average Loss: {avg_loss:.4f}")
