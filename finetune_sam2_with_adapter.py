...
def freeze_parameters_except_adapter(model):
    for name, param in model.named_parameters():
        if "adapter" not in name:  # 冻结非 adapter 参数
            param.requires_grad = False
...
def main():
    ...
    sam2_model = build_sam2(model_cfg, sam2_checkpoint, device=args.device, apply_postprocessing=True,adapter=True)
    ...
    # dummy_input = torch.randn(1, 3, 1024, 1024).to(args.device)
    # output = medsam_model(images[idx],bboxes[idx].detach().cpu().numpy())
    # print(output.shape)
    ...
    adapter_params = [
        param for name, param in medsam_model.named_parameters() if "adapter" in name
    ]
    optimizer = torch.optim.AdamW(adapter_params, lr=args.lr, weight_decay=args.weight_decay)
