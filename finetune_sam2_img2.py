...
def main():
    ...
    medsam_model = torch.nn.parallel.DistributedDataParallel(medsam_model,find_unused_parameters=True)
    medsam_model.train()
    ...
    # cross entropy loss
    ce_loss = nn.BCEWithLogitsLoss(reduction="mean").to(device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (image, gt2D, boxes, _) in enumerate(tqdm(train_dataloader)):
            ...
            image.contiguous()
            image, gt2D = image.to(device, dtype=torch.float32), gt2D.to(device, dtype=torch.float32)
